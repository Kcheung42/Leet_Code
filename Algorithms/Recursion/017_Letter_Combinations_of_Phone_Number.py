...
class SolutionFunctional:
    '''Pure functions'''
    def letterCombinationsRecur(self, lookup, digits, cur, n):
        if n == len(digits):
            return [cur]
        else:
            letters = lookup[int(digits[n])]
            return reduce(
                lambda acc, elem: acc + self.letterCombinationsRecur(
                    lookup, digits, cur + elem, n + 1), letters, [])
    ...
    def letterCombinationsRecur(self, lookup, digits, result, cur, n):
        if n == len(digits):
            ...
            for c in lookup[int(digits[n])]:
                self.letterCombinationsRecur(lookup, digits, result, cur + c,
                                             n + 1)
